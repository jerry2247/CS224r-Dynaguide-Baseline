...
class DiffusionPolicyUNet(PolicyAlgo):
    def _create_networks(self):
        ...
        if len(self.goal_shapes.keys()) > 0:
    ...
    def get_action(self, obs_dict, goal_dict=None, guidance_function = None, guidance_type = None, ss = 4):
        """
        ...
        if len(self.action_queue) == 0:
            ...
            # run inference
            # [1,T,Da]
            # print("I'M SAMPLING FROM THE MAIN SAMPLER (SHOULDNT HAPPEN IF YOU'RE DOING REJECTION SAMPLING)")
            # action_sequence = self._get_action_trajectory_directopt(obs_dict=obs_dict, guidance_function = guidance_function)
    ...
    def _get_action_trajectory(self, obs_dict, goal_dict=None): 
        ...
        for k in self.obs_shapes:
            # first two dimensions should be [B, T] for inputs. If it's not, we just unsqueeze once 
            if inputs['obs'][k].ndim - 1 == len(self.obs_shapes[k]):
                inputs["obs"][k] = torch.unsqueeze(inputs["obs"][k], dim = 1)
            assert inputs['obs'][k].ndim - 2 == len(self.obs_shapes[k])
    ...
    def _get_action_trajectory_directopt(self, obs_dict, goal_dict=None, guidance_function = None):
        ...
        for i in range(50): # prevents overstepping 
            ...
            guidance = guidance_function(state, naction)
            
            # print(torch.norm(guidance).item())
            # scaled_guidance = 0.01 * guidance
    ...
    def _get_action_trajectory_guidance(self, obs_dict, goal_dict=None, guidance_function = None, ss = 4):
        ...
        for k in tqdm.tqdm(self.noise_scheduler.timesteps):
            ...
            for ss_iter in range(ss): # stochastic sampling; if M = 1, it's the same as normal guidance 
                noise_pred = nets['policy']['noise_pred_net'](
                    sample=naction, 
                    timestep=k,
                    global_cond=obs_cond
            ...
            self.diffusion_list.append((1 - self.noise_scheduler.alphas_cumprod[k]).sqrt() * noise_pred.clone())
            # print(k, (1 - self.noise_scheduler.alphas_cumprod[k]).sqrt())
            self.guidance_list.append((1 - self.noise_scheduler.alphas_cumprod[k]).sqrt() * scaled_grad)
            # self.guidance_list.append(scaled_grad)

        self.corrections_list.append(naction.clone()) # add the last correction 
        self.guidance_list.append(torch.zeros_like(naction)) # no correction for the last 
        self.diffusion_list.append(torch.zeros_like(noise_pred)) # no correction for the last 
