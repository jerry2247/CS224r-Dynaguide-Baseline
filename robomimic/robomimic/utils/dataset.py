    ...
    def get_item(self, index):
        ...
        if goal_index is not None:
            goal = self.get_obs_sequence_from_demo(
                ...
                keys=self.obs_keys,
                num_frames_to_stack=0, # used to be 0 but caused problems 
                seq_length=1,
                # prefix="next_obs",
                prefix="obs", # HACK: our datasets don't have next_obs; one step is fine. 
            )
            # meta["goal_obs"] = {k: goal[k][0] for k in goal}  # remove sequence dimension for goal
            meta["goal_obs"] = {k: goal[k] for k in goal}  # remove sequence dimension for goal
