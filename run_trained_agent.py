...
def rollout(policy, env, horizon, render=False, video_writer=None, video_skip=5, return_obs=False, 
            camera_names=None, real=False, rate_measure=None, env_state = None):
    """
    ...
    obs = env.reset()
    # env.env.env.env.env.reset(env_state) # super hacky but this reinstates a state for the pymunk environment 
    state_dict = dict()
    # if real:
    ...
    try:
        for step_i in range(horizon):
            ...
            if not real:
                ...
                traj["states"].append([]) # TODO: for pymunk only 
            if return_obs:
...
def run_trained_agent(args):
    ...
    env.reset() 
    # orig_state = env.env.env.reset_state # janky way of extracting the reset state 
