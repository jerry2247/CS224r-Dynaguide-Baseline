...
class PlayTableScene:
    def __init__(self, objects, data_path, euler_obs, p, cid, global_scaling, surfaces, np_random, recorder = None, **kwargs):
        self.p = p
        self.cid = cid
    ...
    def load(self):
        ...
        for name, obj_cfg in self.object_cfg.get("movable_objects", {}).items():
            ...
            self.movable_objects.append(movable_ojt)              
            if self.recorder is not None:
                self.recorder.register_object(body_id = movable_ojt.uid, urdf_path = movable_ojt.file.as_posix(), global_scaling = self.global_scaling) # CHECK SCALING!!
        ...
        for name, obj_cfg in self.object_cfg.get("fixed_objects", {}).items():
            ...
            if self.recorder is not None:
                self.recorder.register_object(body_id = fixed_obj.uid, urdf_path = fixed_obj.file.as_posix(), global_scaling = self.global_scaling) 
                # print(self.global_scaling)
